...
using ValueSpecifierCase = ::envoy::extensions::filters::http::rate_limit_quota_apig::v3::
...
RateLimitOnMatchAction::generateBucketId(const Http::Matching::HttpMatchingDataImpl& data,
  ...
  // Generate the `BucketId` based on the bucket id builder from the configuration.
